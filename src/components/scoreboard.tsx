...
interface ScoreboardProps {
  ...
  currentRotation: { own: number; rival: number };
  onRotationChange: (team: 'own' | 'rival', rot: number) => void;
}

const TeamScore = ({ name, score, onNameChange, isEditing, icon: Icon, iconColor, hasServe, teamSide, currentRotation, onRotationChange }: {
  name: string,
  score: number,
  ...
  <div className="flex flex-col items-center gap-4 text-center">
    ...
    <div className={cn(
      "flex items-center justify-center gap-2 sm:gap-4 relative",
      teamSide === 'rival' && 'flex-row-reverse'
    )}>
      {/* Rotation Badge as a circular button */}
      <Button
        variant="outline"
        size="icon"
        className={cn(
          "w-8 h-8 rounded-full font-bold text-xs shrink-0 flex items-center justify-center p-0 border-2 transition-all",
          hasServe
            ? (teamSide === 'own' ? "bg-accent text-accent-foreground border-accent shadow-md scale-110" : "bg-destructive text-destructive-foreground border-destructive shadow-md scale-110")
            : "bg-secondary text-secondary-foreground opacity-70 hover:opacity-100 border-transparent"
        )}
        onClick={(e) => {
          e.stopPropagation();
          onRotationChange(teamSide, currentRotation === 1 ? 6 : currentRotation - 1);
        }}
        title={`P${currentRotation}`}
      >
        P{currentRotation}
      </Button>
      ...
      <CardContent className="p-4 sm:p-6">
        <div className="grid grid-cols-[1fr_auto_1fr] items-start gap-2 sm:gap-4">
          <TeamScore
            ...
            teamSide="own"
            currentRotation={currentRotation.own}
          ...
          <TeamScore
            ...
            teamSide="rival"
            currentRotation={currentRotation.rival}
            onRotationChange={onRotationChange}
          />
        </div>
      </CardContent>
